import { CollapseTypes } from "../diagram/component/customizable-attribute";
import { TreeInspector } from "@aws-cdk/core";
export declare class CdkDia {
    static readonly attrPrefix = "CDK-DIA_";
    static decorate(inspector: TreeInspector, decorator: CdkDiaDecorator): void;
}
export declare class CdkDiaDecorator {
    private attributes;
    collapse(val: CollapseTypes): CdkDiaDecorator;
    build(): {
        [key: string]: string;
    };
}
export declare function DiagramOptions(options: DiagramOptionValues): ClassDecorator;
export interface DiagramOptionValues {
    collapse: CollapseTypes;
}
